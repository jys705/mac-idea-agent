"""
의미 유사도용 임베딩 유틸 (Week 3 text-embedding-3-small 재활용).
...
- OPENAI_API_KEY가 없으면 임베딩이 불가하므로 embedding_available()이 False를 반환하고,
...
# None이면 OpenAI 실호출(_openai_embed)을 사용한다.
...
def _openai_embed(texts: Sequence[str]) -> list[list[float]]:
    """text-embedding-3-small 실제 호출 (lazy init)."""
    global _openai_client
    if _openai_client is None:
        from langchain_openai import OpenAIEmbeddings

        _openai_client = OpenAIEmbeddings(model=EMBED_MODEL)
    return _openai_client.embed_documents(list(texts))


def embed(texts: Sequence[str]) -> list[list[float]]:
    """주입된 함수 우선, 없으면 OpenAI 실호출로 임베딩한다."""
    fn = _EMBED_FN or _openai_embed
    return fn(texts)
