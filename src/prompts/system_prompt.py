...
2. **app_existence_checker는 의미 유사도(임베딩)로 판정한다. 검색이 잘 되도록
   ...
   - similar_app_found=True이면 exclude_concepts에 해당 앱명을 추가하여 concept_generator를 재호출한다.
   - 점수 구간(검사관이 자동 처리):
     · 유사도 ≥ 0.85 → 명백한 중복 → similar_app_found=True (자동 루프백)
     · 0.65~0.85 → 애매 → 검사관이 사용자에게 근거를 보여주고 확인을 받는다(Human-in-the-loop).
  ...
  "today_brief": {{
    ...
    "concepts": [
      {{
        ...
        "stack": ["Swift", "AppKit"],
        "kickoff": {{
          "spec_path": "output/앱이름/SPEC.md",
          "brief_path": "output/앱이름/BRIEF.md"
        }}
      }}
    ]
  }},
...
각 concept의 similarity_score는 app_existence_checker가 반환한 값을, kickoff.spec_path/brief_path는
feasibility_checker가 반환한 spec_path/brief_path를 그대로 채운다. (없으면 생략 가능)
